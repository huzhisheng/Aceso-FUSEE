...
public:
    ...
    uint64_t raw_valid_kv_sz_;
    uint64_t cur_valid_kv_sz_;  // for memory consumption test
    uint64_t max_valid_kv_sz_;  // for memory consumption test
...
public:
    inline void print_allocated_kv_sz() {
        printf("cur_valid_kv_sz_: %lld\n", cur_valid_kv_sz_);
        printf("max_valid_kv_sz_: %lld\n", max_valid_kv_sz_);
    }

    inline void get_allocated_kv_sz(uint64_t * cur_sz, uint64_t * max_sz, uint64_t * raw_sz) {
        *cur_sz = cur_valid_kv_sz_;
        *max_sz = max_valid_kv_sz_;
        *raw_sz = raw_valid_kv_sz_;
    }
