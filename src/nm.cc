...
int UDPNetworkManager::UDPCMInitClient(const struct GlobalConfig * conf) {
    ...
    timeout.tv_sec = 0;
    timeout.tv_usec = CLIENT_RETRY_INTERVAL;
    setsockopt(udp_sock_, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(struct timeval));
...
int UDPNetworkManager::client_connect_one_rc_qp(uint32_t server_id) {
    ...
    while (retry_count < 5 && !received_reply) {
        // send request
        ...
        if (rc == sizeof(struct KVMsg)) {
            ...
            printf("Failed to recv request.\n");
            retry_count++;
        }
    }
    if (!received_reply) {
        printf("Failed to do client_connect_one_rc_qp.\n");
        exit(1);
    } 
...
int UDPNetworkManager::nm_send_and_recv_udp_msg_to_server(struct KVMsg * request, struct KVMsg * reply, uint32_t server_id) {
    int rc = 0;
    int retry_count = 0;
    bool received_reply = false;
    while (retry_count < 5 && !received_reply) {
    ...
    if (!received_reply) {
        printf("Failed to do nm_send_and_recv_udp_msg_to_server\n");
        exit(1);
    }
    return 0;
}
...
void UDPNetworkManager::nm_thread_polling() {
    ...
    while (stop_polling_ == false) {
        ...
        for (int i = 0; i < polled_num; i ++) {
            ...
            struct ibv_wc * store_wc = (struct ibv_wc *)0xFF;
...
void UDPNetworkManager::set_is_polling_started() {
    is_polling_started_ = true;
}
