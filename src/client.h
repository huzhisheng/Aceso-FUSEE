    ...
    inline void update_cache(std::string key_str, RaceHashSlot * slot_info, uint64_t * r_slot_addr_list) {
        ...
        if (it != addr_cache_.end()) {
            ...
            if (*(uint64_t *)(&entry->l_slot_ptr) != *(uint64_t *)slot_info) {
                RaceHashSlot * cached_slot_info = (RaceHashSlot *)(&entry->l_slot_ptr);
                RaceHashSlot * new_slot_info = slot_info;
                // should not add miss_cnt here
                // entry->miss_cnt ++; 
...
private:
    ...
    bool cache_entry_is_valid(KVReqCtx * ctx);
...
public:
    ...
    // for checkpoint
    uint64_t debug_cas_count_;

    uint64_t prepare_request_us_all = 0;
    uint64_t kv_search_read_buckets_sync_us_all = 0;
    uint64_t kv_search_read_kv_sync_us_all = 0;
    uint64_t kv_search_check_kv_us_all = 0;
    uint64_t kv_search_count = 0;
    uint64_t kv_search_cache_hit_count = 0;
    uint64_t kv_search_cache_enter_count = 0;

    bool stop_gc_;

    bool init_is_finished();
    int  sync_init_finish();
    void wait_load_finished();
    void wait_load_finished(uint32_t wait_num);
    void wait_micro_finished(uint32_t CN_num, uint32_t client_num_per_CN, uint32_t expected_val);
    int  sync_load_finish();
    int  sync_load_finish(uint32_t wait_num);
    int  sync_micro_finish(uint32_t expected_val);
...
public:
    ...
    inline void print_consumption() {
        mm_->print_allocated_kv_sz();
    }

    inline void get_consumption(uint64_t * cur_sz, uint64_t * max_sz, uint64_t * raw_sz) {
        mm_->get_allocated_kv_sz(cur_sz, max_sz, raw_sz);
    }
};
