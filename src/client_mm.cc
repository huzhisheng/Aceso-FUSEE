...
ClientMM::ClientMM(const struct GlobalConfig * conf, UDPNetworkManager * nm) {
    ...
    server_kv_area_off_ = HASH_AREA_BORDER;
    ...
    cur_valid_kv_sz_ = 0;
    max_valid_kv_sz_ = (server_num_blocks_ * num_memory_) / num_replication_ * ((subblock_num_ - bmap_block_num_) * subblock_sz_);
    raw_valid_kv_sz_ = 0;
...
int ClientMM::init_get_new_block_from_server(UDPNetworkManager * nm) {
    ...
    for (int i = 0; i < num_memory_; i ++) {
        ...
        printf("get a block form %d, addr %lx\n", pr_server_id, mr_info_list[i][0].addr);
...
void ClientMM::mm_alloc(size_t size, UDPNetworkManager * nm, __OUT ClientMMAllocCtx * ctx) {
    ...
    cur_valid_kv_sz_ += aligned_size;
    raw_valid_kv_sz_ += aligned_size * 3;
...
int ClientMM::alloc_from_sid(uint32_t server_id, UDPNetworkManager * nm, int alloc_type,
    ...
    int ret = nm->nm_send_and_recv_udp_msg_to_server(&request, &reply, server_id);
    assert(ret == 0);
