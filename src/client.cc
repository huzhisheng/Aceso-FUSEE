...
void * Client::kv_search_sync(KVReqCtx * ctx) {
    // print_log(DEBUG, "[%s fb%d %ld] start", __FUNCTION__, ctx->coro_id, boost::this_fiber::get_id());
    timeval st, et;
    uint64_t prepare_request_us;
    uint64_t kv_search_read_buckets_sync_us;
    uint64_t kv_search_read_kv_sync_us;
    uint64_t kv_search_check_kv_us;

    gettimeofday(&st, NULL);
    prepare_request(ctx);
    gettimeofday(&et, NULL);
    prepare_request_us = time_spent_us(&st, &et);
    
    gettimeofday(&st, NULL);
    kv_search_read_buckets_sync(ctx);
    gettimeofday(&et, NULL);
    kv_search_read_buckets_sync_us = time_spent_us(&st, &et);

    gettimeofday(&st, NULL);
    kv_search_read_kv_sync(ctx);
    gettimeofday(&et, NULL);
    kv_search_read_kv_sync_us = time_spent_us(&st, &et);

    gettimeofday(&st, NULL);
    kv_search_check_kv(ctx);
    gettimeofday(&et, NULL);
    kv_search_check_kv_us = time_spent_us(&st, &et);

    // printf("%lld, %lld, %lld, %lld\n", prepare_request_us, kv_search_read_buckets_sync_us, kv_search_read_kv_sync_us, kv_search_check_kv_us);
    prepare_request_us_all += prepare_request_us;
    kv_search_read_buckets_sync_us_all += kv_search_read_buckets_sync_us;
    kv_search_read_kv_sync_us_all += kv_search_read_kv_sync_us;
    kv_search_check_kv_us_all += kv_search_check_kv_us;
    kv_search_count++;
...
IbvSrList * Client::gen_read_cache_kv_sr_lists(uint32_t coro_id, RaceHashSlot * local_slot_ptr, uint64_t local_addr) {
    ...
    this->req_search_num[local_slot_ptr->server_id] ++;
...
void Client::set_numa_policy() {
    ...
    for(int i = 0; i < 2; i++) {
        if(numa_bitmask_isbitset(current_mask, i))
            printf("Current memory binding: %d\n", i);
    }
...
int Client::sync_load_finish() {
    uint64_t local_msg = 1;
    int ret = 0;
    for (int i = num_memory_ - 1; i >= 0; i --) {
        ret = nm_->nm_rdma_write_inl_to_sid(&local_msg, sizeof(uint64_t), 
            remote_global_meta_addr_ + sizeof(uint64_t), server_mr_info_map_[i]->rkey, i);
        assert(ret == 0);
    }
    return 0;
}

int Client::sync_load_finish(uint32_t wait_num) {
    uint64_t local_msg = 1;
    int ret = 0;
    wait_num += 10; // not collicted with sync_load_finish()
    for (int i = num_memory_ - 1; i >= 0; i --) {
        ret = nm_->nm_rdma_write_inl_to_sid(&local_msg, sizeof(uint64_t), 
            remote_global_meta_addr_ + sizeof(uint64_t) * wait_num, server_mr_info_map_[i]->rkey, i);
        assert(ret == 0);
    }
    return 0;
}

// only used for micro_test_multi_client
int Client::sync_micro_finish(uint32_t expected_val) {
    uint64_t local_msg = 1;
    int ret = 0;
    for (int i = num_memory_ - 1; i >= 0; i --) {
        ret = nm_->nm_rdma_write_inl_to_sid(&local_msg, sizeof(uint64_t), 
            remote_global_meta_addr_ + 2048 * expected_val + sizeof(uint64_t) * my_server_id_, server_mr_info_map_[i]->rkey, i);
        assert(ret == 0);
    }
    return 0;
}
...
void Client::wait_load_finished() {
    int ret = 0;
    memset(local_buf_, 0, sizeof(uint64_t) * 16);
    uint64_t read_value = *(uint64_t *)local_buf_;
    while(read_value == 0) {
        for(int m = 0; m < num_memory_; m++) {
            assert(local_buf_ != 0);
            ret = nm_->nm_rdma_read_from_sid(local_buf_, local_buf_mr_->lkey, 
                sizeof(uint64_t), remote_global_meta_addr_ + sizeof(uint64_t), server_mr_info_map_[m]->rkey, m);
            assert(ret == 0);
        }
        read_value = *(uint64_t *)local_buf_;
    }
    assert(read_value == 1);
    return;
}

void Client::wait_load_finished(uint32_t wait_num) {
    int ret = 0;
    memset(local_buf_, 0, sizeof(uint64_t) * 16);
    uint64_t read_value = *(uint64_t *)local_buf_;
    ...
    while(read_value == 0) {
        for(int m = 0; m < num_memory_; m++) {
            assert(local_buf_ != 0);
            ret = nm_->nm_rdma_read_from_sid(local_buf_, local_buf_mr_->lkey, 
                sizeof(uint64_t), remote_global_meta_addr_ + sizeof(uint64_t) * wait_num, server_mr_info_map_[m]->rkey, m);
            assert(ret == 0);
        }
        read_value = *(uint64_t *)local_buf_;
    }
...
// only used for micro_test_multi_client, 19 CNs
void Client::wait_micro_finished(uint32_t CN_num, uint32_t client_num_per_CN, uint32_t expected_val) {
    int ret = 0;
    memset(local_buf_, 0, sizeof(uint64_t) * 16);
    uint64_t read_value = *(uint64_t *)local_buf_;

    for(uint32_t other_client_id = num_memory_; other_client_id < num_memory_ + CN_num * 8;  other_client_id += 8) {
        while(read_value == 0) {
            for(int m = 0; m < num_memory_; m++) {
                assert(local_buf_ != 0);
                ret = nm_->nm_rdma_read_from_sid(local_buf_, local_buf_mr_->lkey, 
                    sizeof(uint64_t), remote_global_meta_addr_ + 2048 * expected_val + sizeof(uint64_t) * other_client_id, server_mr_info_map_[m]->rkey, m);
                assert(ret == 0);
            }
            read_value = *(uint64_t *)local_buf_;
        }
...
int32_t Client::find_match_kv_idx(KVReqCtx * ctx) {
    ...
    for (size_t i = 0; i < ctx->kv_read_addr_list.size(); i ++) {
        ...
        if (CheckKey((void *)read_key_addr, header->key_length, (void *)local_key_addr, ctx->kv_info->key_len)) {
            ctx->kv_wait_addr = ctx->kv_read_addr_list[i];
            return i;
        }
    }
    return -1;
...
void Client::modify_backup_idx_consensus_1(KVReqCtx * ctx) {
    ...
    if (ctx->consensus_state == KV_CONSENSUS_FAIL) {
        ...
        do {
            ...
            // post_sr_lists_and_yield_wait(read_pr_sr_list, 1, ctx->should_stop);
    ...
    // ret = post_sr_list_batch_and_yield_wait(kv_insert_p2_sr_list_batch, kv_insert_p2_sr_list_num_batch, ctx->should_stop);
...
void Client::modify_backup_idx_consensus_1_sync(KVReqCtx * ctx) {
    ...
    if (ctx->consensus_state == KV_CONSENSUS_FAIL) {
        ...
        } while (*(uint64_t *)ctx->kv_modify_pr_cas_list[0].l_kv_addr == ctx->kv_modify_pr_cas_list[0].orig_value);
        uint8_t val = KV_OP_FINISH;
        uint32_t num_sr_list;
        IbvSrList * write_unused_sr_list = gen_log_commit_sr_lists(ctx->coro_id,
            &val, sizeof(uint8_t), ctx->write_unused_addr_list, &num_sr_list);
...
bool Client::cache_entry_is_valid(KVReqCtx * ctx) {
    get_local_bucket_info(ctx);

    RaceHashSlot * local_read_slot;
    LocalCacheEntry * cache_entry = ctx->cache_entry;
    assert(cache_entry != NULL);

    KVLogHeader * cached_header = (KVLogHeader *)ctx->local_cache_addr;

    // in first combined bucket
    if (cache_entry->r_slot_addr[0] >= ctx->tbl_addr_info.f_bucket_addr[0] && 
        cache_entry->r_slot_addr[0] < ctx->tbl_addr_info.f_bucket_addr[0] + (sizeof(RaceHashBucket) * 2)) {
        uint64_t slot_off = cache_entry->r_slot_addr[0] - ctx->tbl_addr_info.f_bucket_addr[0];
        local_read_slot = (RaceHashSlot *)(((uint64_t)ctx->f_com_bucket) + slot_off);
    }
    else {
        assert(cache_entry->r_slot_addr[0] >= ctx->tbl_addr_info.s_bucket_addr[0]);
        assert(cache_entry->r_slot_addr[0] < (ctx->tbl_addr_info.s_bucket_addr[0] + (sizeof(RaceHashBucket) * 2)));
        uint64_t slot_off = cache_entry->r_slot_addr[0] - ctx->tbl_addr_info.s_bucket_addr[0];
        local_read_slot = (RaceHashSlot *)(((uint64_t)ctx->s_com_bucket) + slot_off);
    }

    uint64_t local_read_slot_val = ConvertSlotToInt(local_read_slot);
    uint64_t cache_read_slot_val = ConvertSlotToInt(&cache_entry->l_slot_ptr);

    bool has_old = false;
    for(int i = 0; i < RACE_HASH_ASSOC_NUM; i++) {
        has_old =  (ConvertSlotToInt(&(ctx->slot_arr[0][i])) == cache_read_slot_val)
                || (ConvertSlotToInt(&(ctx->slot_arr[1][i])) == cache_read_slot_val)
                || (ConvertSlotToInt(&(ctx->slot_arr[2][i])) == cache_read_slot_val)
                || (ConvertSlotToInt(&(ctx->slot_arr[3][i])) == cache_read_slot_val);
        if(has_old) break;
    }

    bool ret = (local_read_slot_val == cache_read_slot_val);
    if(ret != log_is_valid(cached_header)) {
        printf("ret != log_is_valid %llx, %llx, has_old %d\n", local_read_slot_val, cache_read_slot_val, has_old);
    }
...
void Client::modify_primary_idx(KVReqCtx * ctx) {
    ...
    // ret = post_sr_list_batch_and_yield_wait(phase2_sr_list_batch, phase2_sr_list_num_batch, ctx->should_stop);
...
void Client::kv_delete_read_buckets_write_log(KVReqCtx * ctx) {
    ...
    // 3. post requests
    // ret = post_sr_list_batch_and_yield_wait(phase1_sr_list_batch, phase1_sr_list_num_batch, ctx->should_stop);
...
void * client_ops_fb_cnt_ops(void * arg) {
    ...
    uint64_t needed_cnt = 0;
    uint64_t server_index_access_cnt[5] = {0};
    uint64_t server_kv_access_cnt[5] = {0};
    while (*fiber_args->should_stop == false && fiber_args->ops_num != 0) {
        ...
        case KV_REQ_SEARCH:
            ...
            // if (search_addr != NULL) {
            //     needed_cnt++;
            //     server_index_access_cnt[ctx->tbl_addr_info.server_id_list[0]]++;
            //     server_kv_access_cnt[ctx->kv_wait_addr.server_id]++;
            // }
            break;
        case KV_REQ_INSERT:
            ...
            // if (ret == KV_OPS_SUCCESS) {
            //     needed_cnt++;
            //     server_index_access_cnt[ctx->tbl_addr_info.server_id_list[0]]++;
            //     server_kv_access_cnt[ctx->mm_alloc_ctx.server_id_list[0]]++;
            //     server_kv_access_cnt[ctx->mm_alloc_ctx.server_id_list[1]]++;
            //     server_kv_access_cnt[ctx->mm_alloc_ctx.server_id_list[2]]++;
            // }
    ...
    // fiber_args->ops_cnt = needed_cnt;
    fiber_args->num_failed = num_failed;

    // printf("server_index_access_cnt %lld %lld %lld %lld %lld\n", server_index_access_cnt[0],
    //     server_index_access_cnt[1], server_index_access_cnt[2], server_index_access_cnt[3], server_index_access_cnt[4]);
    // printf("server_kv_access_cnt %lld %lld %lld %lld %lld\n", server_kv_access_cnt[0],
    //     server_kv_access_cnt[1], server_kv_access_cnt[2], server_kv_access_cnt[3], server_kv_access_cnt[4]);
    return NULL;
}

void * client_ops_fb_cnt_lat(void * arg) {
    boost::this_fiber::yield();
    ClientFiberArgs * fiber_args = (ClientFiberArgs *)arg;
    fiber_args->client->init_kvreq_space(fiber_args->coro_id, fiber_args->ops_st_idx, fiber_args->ops_num);
    uint32_t num_failed = 0;
    ...
    std::vector<uint64_t> delete_lat_vec;
    timeval st, et;
    ...
    while (*fiber_args->should_stop == false && fiber_args->ops_num != 0) {
        ...
        case KV_REQ_SEARCH:
            gettimeofday(&st, NULL);
            search_addr = fiber_args->client->kv_search_sync(ctx);
            gettimeofday(&et, NULL);
        ...
        case KV_REQ_INSERT:
            ...
            gettimeofday(&st, NULL);
            do {
                ret = fiber_args->client->kv_insert_sync(ctx);
        ...
        case KV_REQ_UPDATE:
            gettimeofday(&st, NULL);
            ret = fiber_args->client->kv_update_sync(ctx);
            gettimeofday(&et, NULL);
        ...
        case KV_REQ_DELETE:
            gettimeofday(&st, NULL);
            fiber_args->client->kv_delete_sync(ctx);
            gettimeofday(&et, NULL);
            delete_lat_vec.push_back(time_spent_us(&st, &et));
            if (ret == KV_OPS_FAIL_RETURN) {
                num_failed ++;
            }
            break;
        default:
            printf("error req_type.\n");
            exit(1);
        }
        cnt ++;
    ...
    uint64_t prepare_request_us = fiber_args->client->prepare_request_us_all / fiber_args->client->kv_search_count;
    uint64_t kv_search_read_buckets_sync_us = fiber_args->client->kv_search_read_buckets_sync_us_all / fiber_args->client->kv_search_count;
    uint64_t kv_search_read_kv_sync_us = fiber_args->client->kv_search_read_kv_sync_us_all / fiber_args->client->kv_search_count;
    uint64_t kv_search_check_kv_us = fiber_args->client->kv_search_check_kv_us_all / fiber_args->client->kv_search_count;
    uint64_t kv_search_sync_avg_us = (fiber_args->client->prepare_request_us_all + 
                                      fiber_args->client->kv_search_read_buckets_sync_us_all + 
                                      fiber_args->client->kv_search_read_kv_sync_us_all +
                                      fiber_args->client->kv_search_check_kv_us_all) / fiber_args->client->kv_search_count;
    uint64_t kv_search_cache_hit_ratio = fiber_args->client->kv_search_cache_hit_count * 100 / fiber_args->client->kv_search_count;
    uint64_t kv_search_cache_enter_ratio = fiber_args->client->kv_search_cache_enter_count * 100 / fiber_args->client->kv_search_count;
    printf("finally %lld, %lld, %lld, %lld, avg %lld, cache hit ratio %lld\%, cache enter ratio %lld\%\n", 
        prepare_request_us, kv_search_read_buckets_sync_us, kv_search_read_kv_sync_us, 
        kv_search_check_kv_us, kv_search_sync_avg_us, kv_search_cache_hit_ratio, kv_search_cache_enter_ratio);

    char search_fname_buf[128];
    char update_fname_buf[128];
    char insert_fname_buf[128];
    char delete_fname_buf[128];
    ...
    sprintf(delete_fname_buf, "results/ycsb_delete_lat_%d_%d.txt", fiber_args->thread_id, fiber_args->coro_id);
    ...
    dump_lat_file(delete_fname_buf, delete_lat_vec);
...
void * client_ops_fb_cnt_ops_micro(void * arg) {
    ...
    // printf("fb %d exit loop\n", fiber_args->coro_id);
    fiber_args->ops_cnt = cnt;
    fiber_args->num_failed = num_failed;

    // make sure that insert enough KVs
...
void * client_ops_fb_cnt_lat_micro(void * arg) {
    ClientFiberArgs * fiber_args = (ClientFiberArgs *)arg;
    fiber_args->client->init_kvreq_space(fiber_args->coro_id, fiber_args->ops_st_idx, fiber_args->ops_num);
    uint32_t num_failed = 0;
    int ret = 0;
    ...
    uint8_t req_type = KV_REQ_SEARCH;
    while (*fiber_args->should_stop == false && fiber_args->ops_num != 0) {
        uint32_t idx = cnt % fiber_args->ops_num;
        KVReqCtx * ctx = &fiber_args->client->kv_req_ctx_list_[idx + fiber_args->ops_st_idx];
        ctx->coro_id = fiber_args->coro_id;
        ...
        case KV_REQ_DELETE:
            ...
            gettimeofday(&st, NULL);
            ret = fiber_args->client->kv_delete_sync(ctx);
            gettimeofday(&et, NULL);
            delete_lat_vec.push_back(time_spent_us(&st, &et));
            if (ret == KV_OPS_FAIL_RETURN) {
                num_failed ++;
    ...
    if (req_type == KV_REQ_INSERT) {
        char insert_fname_buf[128];
        sprintf(insert_fname_buf, "results/micro_insert_lat_%d_%d.txt", fiber_args->thread_id, fiber_args->coro_id);
        dump_lat_file(insert_fname_buf, insert_lat_vec);
    } else if (req_type == KV_REQ_UPDATE) {
        char update_fname_buf[128];
        sprintf(update_fname_buf, "results/micro_update_lat_%d_%d.txt", fiber_args->thread_id, fiber_args->coro_id);
        dump_lat_file(update_fname_buf, update_lat_vec);
    } else if (req_type == KV_REQ_SEARCH) {
        char search_fname_buf[128];
        sprintf(search_fname_buf, "results/micro_search_lat_%d_%d.txt", fiber_args->thread_id, fiber_args->coro_id);
        dump_lat_file(search_fname_buf, search_lat_vec);
    } else if (req_type == KV_REQ_DELETE) {
        char delete_fname_buf[128];
        sprintf(delete_fname_buf, "results/micro_delete_lat_%d_%d.txt", fiber_args->thread_id, fiber_args->coro_id);
        dump_lat_file(delete_fname_buf, delete_lat_vec);
    }
...
int Client::load_seq_kv_requests(uint32_t num_keys, char * op_type) {
    ...
    for (int i = 0; i < num_local_operations_; i ++) {
        sprintf(key_buf, "micro-%03d-%02d", my_server_id_, i);
        sprintf(value_buf, "initial-value-%d", i);
...
void * client_ops_fb_cnt_consumption(void * arg) {
    boost::this_fiber::yield();
    ClientFiberArgs * fiber_args = (ClientFiberArgs *)arg;
    fiber_args->client->init_kvreq_space(fiber_args->coro_id, fiber_args->ops_st_idx, fiber_args->ops_num);
    uint32_t num_failed = 0;
    ...
    std::unordered_map<std::string, bool> inserted_key_map;
    for (int i = 0; i < fiber_args->target_ops_cnt; i ++) {
        uint32_t idx = i % fiber_args->ops_num;
        KVReqCtx * ctx = &fiber_args->client->kv_req_ctx_list_[idx + fiber_args->ops_st_idx];
        ctx->coro_id = fiber_args->coro_id;
        ctx->should_stop = fiber_args->should_stop;
        ctx->is_finished = false;
