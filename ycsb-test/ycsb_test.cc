...
int is_valid_workload(char * workload_name) {
    return strcmp(workload_name, "workloada") == 0 || strcmp(workload_name, "workloadb") == 0 ||
           ...
           strcmp(workload_name, "workloadtwis") == 0 || strcmp(workload_name, "workloadtwic") == 0 ||
           strcmp(workload_name, "workloadtwit") == 0;
}
...
int load_workload_sync(Client & client, WorkloadFileName * workload_fnames) {
    ...
    struct timeval thread_st, thread_et;
    gettimeofday(&thread_st, NULL);
    ...
    gettimeofday(&thread_et, NULL);

    printf("Load phase ends (Failed %d)\n", num_failed);
    uint64_t time_spent_us = (thread_et.tv_sec - thread_st.tv_sec) * 1000000 + (thread_et.tv_usec - thread_st.tv_usec);
    printf("time spent: %lf ms\n", (float)time_spent_us / 1000);
    return 0;
}
...
void * run_client_lat(void * _args) {
    ...
    client.clear_cache();
    ret = load_test_lat_1coro(client, workload_fnames, args);
    assert(ret == 0);
...
int load_test_lat_1coro(Client & client, WorkloadFileName * workload_fname, RunClientArgs * args) {
    int ret = 0;
    int num_coro = 1;
    ...
    printf("Test phase start\n");
    boost::fibers::barrier global_barrier(num_coro + 1);
    ClientFiberArgs * fb_args_list = (ClientFiberArgs *)malloc(sizeof(ClientFiberArgs) * num_coro);
    uint32_t coro_num_ops = client.num_local_operations_ / num_coro;
    for (int i = 0; i < num_coro; i ++) {
        fb_args_list[i].client = &client;
    ...
    boost::fibers::fiber fb_list[num_coro];
    for (int i = 0; i < num_coro; i ++) {
        boost::fibers::fiber fb(client_ops_fb_cnt_lat, &fb_args_list[i]);
        fb_list[i] = std::move(fb);
    }
